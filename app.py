...
st.markdown(
    ...
    div[data-testid="stMetric"] {
        ...
        border-radius: 16px;
        padding: 16px 14px;
        box-shadow: 0 2px 10px rgba(123, 31, 162, 0.06);
    }
...
def segment_cards(result):
    ...
    for col, name in zip([c1, c2, c3], counts):
        ...
        with col:
            ...
            st.markdown(
                f"""
                <div class="segment-card" style="border-left-color:{colors[name]};">
...
with st.sidebar.expander("Chart Options"):
    ...
    show_box = st.checkbox("Box Plot", True)
...
if excluded:
    st.caption(
        f"Skipped id-like / constant columns: {', '.join(excluded)}."
    )
...
with left:
    ...
    if st.button("Find Best K (Elbow Method)", use_container_width=True):

        X, _ = preprocess_data(df, features)

        best_k, wcss, silhouette = find_best_k(X)

        st.success(f"Suggested K = {best_k}   ·   Silhouette = {round(silhouette, 3)}")
...
section_header("3 · Results")

m1, m2 = st.columns([1, 3])

m1.metric("Silhouette Score", round(pipeline["silhouette"], 3))

with m2:

    st.markdown(
        """
        <div style="padding-top:16px; color:#6b7280; font-size:0.9rem;">
            Values close to <b>1</b> mean well-separated, compact clusters.
        </div>
        """,
        unsafe_allow_html=True
    )
...
section_header("4 · Search & Insights")
...
# -------------------------------------------------------
# 5. Predict New Customer
# -------------------------------------------------------

section_header("5 · Predict New Customer")
